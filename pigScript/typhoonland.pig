...
Land_data = FOREACH group_data {GENERATE group, FLATTEN(filter_data.power) AS power ,FLATTEN(filter_data.lat) AS lat , FLATTEN(filter_data.log) AS log ,FLATTEN(filter_data.num) AS year,FLATTEN(filter_data.month) AS month,FLATTEN(filter_data.day) AS day,FLATTEN(filter_data.time) AS time;};

-- outputデータを削除
rmf data/output/land;
-- 取得したデータをoutput
;STORE Land_data INTO 'data/output/land' USING PigStorage(',');